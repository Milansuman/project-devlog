...
export const blogTag = pgTable("blogTag", {
  id: t.serial().primaryKey(),
  blogId: t.integer().notNull().references(() => blog.id, {
    onDelete: "cascade"
  }),
  tagId: t.integer().notNull().references(() => tag.id, {
    onDelete: "cascade"
  })
}, (table) => [
  t.uniqueIndex("blog_tag_idx").on(table.blogId, table.tagId)
]);
